...
public class ImageCycleView extends LinearLayout {
	 ...
	 * 广告轮播时间
	 */
	private long rollTime = 5000;
	
	/**
	 * 广告小圆点的距离
	 */
	private int left_dis = 8;
	private int right_dis = 8;
	...
	public ImageCycleView(Context context, AttributeSet attrs) {
		...
		tv = (TextView) findViewById(R.id.guanggao_tv);
	...
	public void setImageResources(ArrayList<ADInfo> infoList, ImageCycleViewListener imageCycleViewListener) {
		...
		for (int i = 0; i < imageCount; i++) {
			...
			LayoutParams layout = new LayoutParams(15, 15);
			layout.setMargins(left_dis, 0, right_dis, 0);
			mImageView.setLayoutParams(layout);
			//mImageView.setPadding(imagePadding, imagePadding, imagePadding, imagePadding);
			mImageViews[i] = mImageView;
			if (i == 0) {
				mImageViews[i].setBackgroundResource(R.drawable.img_guide_dot_selected);
			} else {
				mImageViews[i].setBackgroundResource(R.drawable.img_guide_dot_normal);
			}
	...
	private void startImageTimerTask() {
		stopImageTimerTask();
		// 图片每3秒滚动一次
		mHandler.postDelayed(mImageTimerTask, rollTime );
	}
	...
	private final class GuidePageChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int index) {
			...
			mImageIndex = index;
			index -= 1;
			mImageViews[index].setBackgroundResource(R.drawable.img_guide_dot_selected);
	...
	private class ImageCycleAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, final int position) {
			...
			tv.setText(mAdList.get(position).getContent());
