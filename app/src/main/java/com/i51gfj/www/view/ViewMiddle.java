...
public class ViewMiddle extends LinearLayout implements ViewBaseAction {
	...
	public List<ShopBean.CateListBean> data;
	...
	private String showString = "全部分类";
	int firstPosizion =0;
	...
	private void init(Context context) {
	  ...
	  void 	setItemValue(Context context){
		  groups.clear();
		for(int i=0;i<data.size();i++){
			//groups.add(items[i].toString());
			groups.add(data.get(i).getName());
			LinkedList<String> tItem = new LinkedList<String>();
			List<ShopBean.CateListBean.ListBean> temp = data.get(i).getList();
			for(int j=0,n=temp.size();j<n;j++){
				tItem.add(temp.get(j).getName());
			}
				...
				.setOnItemClickListener(new TextAdapter.OnItemClickListener() {
					...
					public void onItemClick(View view, final int position) {
						...
						if (mOnSelectListener != null) {
							...
							mOnSelectListener.getValue(showString,firstPosizion,position);
						}
	...
	public void setData(List<ShopBean.CateListBean> cateList,Context context) {
		data = cateList;
		setItemValue(context);
	}
