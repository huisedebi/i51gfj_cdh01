...
public class JpushReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
			...
			String params = getBundleData(bundle);
			if (params == null) {
				params = "";
			}
			...
			/*String url = "http://fujin.dingzhijl.com/index.php?g=naichawei&m=article&a=page_app" + params;
			Log.e("url", url);
			Intent startIntent = new Intent(context, MyWebViewActivity.class);
			startIntent.putExtra("title", "信息通知");
	...
	private static String printBundle(Bundle bundle) {
		...
		for (String key : bundle.keySet()) {
			if (key.equals(JPushInterface.EXTRA_NOTIFICATION_ID)) {
				sb.append("\n1key:" + key + ", value:" + bundle.getInt(key));
			} else if (key.equals(JPushInterface.EXTRA_CONNECTION_CHANGE)) {
				sb.append("\n2key:" + key + ", value:" + bundle.getBoolean(key));
			} else {
				sb.append("\n3key:" + key + ", value:" + bundle.getString(key));
			}
		}
		return sb.toString();
	}
	...
	private static String getBundleData(Bundle bundle) {
		...
		for (String key : bundle.keySet()) {
			if (key.equals("cn.jpush.android.EXTRA")) {
				Map<String, String> map = new Gson().fromJson(bundle.getString(key), Map.class);
